...
class IeJs(object):
    ...
    function createXPathFromElement(elm) { 
        ...
        if (elm.getAttribute('id')) {myId = elm.getAttribute('id');}
        if(elm.getAttribute('class')){myClass= elementTagLower.toLowerCase() + "[class='" + elm.getAttribute('class') + "']";}
    ...
    {
        element = evt.target || evt.srcElement;   // not IE
    ...
    function DIOnMouseDown(ev)
    {
        ...
        if(ev.ctrlKey){
            ...
            xpath = createXPathFromElement(ev);
             ...
             myinput2.setAttribute("value",encodeURI(getValue(ev)));
            ...
            myinput7.setAttribute("type","hidden");
             ...
             myinput7.setAttribute("value",encodeURI(currentFrame.join()));
            ...
            myinput8.setAttribute("type","hidden");
             ...
             myinput8.setAttribute("value",currentWindowType.join());
            ...
            myinput9.setAttribute("type","hidden");
             ...
             myinput9.setAttribute("value",allIndexFrame.join());
            ...
            if(ev.target.localName.toLowerCase()=="a"){
                ev = ev.target || ev.srcElement;
                href = ev.href;
                location.href = href;
            }
    ...
    function setIntervalForMain(){
        ...
        for(var i=0;i<nodes.length;i++){  
            var o = nodes[i];
            if (o.getAttribute('uniquedFrame')){
                o.getAttribute('uniquedFrame');
            }else if (o.getAttribute('uniquedFlag')){
                    o.getAttribute('uniquedFlag');
                }else{
        ...
        forEveryframe(document);
    }
    
    function forEveryIframe(loopDoc){
        var iframes = loopDoc.getElementsByTagName('iframe');
        for(var i=0; i<iframes.length; i++){
            handleEveryFrame(iframes[i]);
        }
    }
    
    function forEveryframe(loopDoc){
        var iframes = loopDoc.getElementsByTagName('frame');
        for(var i=0; i<iframes.length; i++){
           handleEveryFrame(iframes[i]);
        }
    ...
    function handleEveryFrame(curIfr){
        if(curIfr){
    ...
    function readyDom() {
        ...
        try{
            var iframeElement = document.createElement('<iframe name="abcdefg" style="display: none;width:0;height:0;" uniquedFrame="weiyi">');
        }catch(e){
            var iframeElement = document.createElement("iframe");
            iframeElement.offWidth = 0;
            iframeElement.offHeight=0;
            iframeElement.setAttribute("name","abcdefg");
            iframeElement.setAttribute("style","display:none;");
            iframeElement.setAttribute("uniquedFrame","weiyi");
        }
        ...
        formElement.setAttribute("uniquedFrame","weiyi");
        ...
        try{
            ...
            try{
                var newBody = document.createElement('<body uniquedFrame="weiyi"></body>');
            }catch(e){
                var newBody = document.createElement('body');
                newBody.setAttribute("uniquedFrame","weiyi");
            }
            newBody.appendChild(iframeElement);
            newBody.appendChild(formElement);
